...
const msg = {
  ...
  argOptionsPojo: 'The argument "options" must be a POJO',
  argOptionsDisposeBool: 'Argument "options.dispose" must be a Boolean',
};
...
function assertConnectorOptions(options) {
  const defaults = { dispose: false };

  if (!elv(options)) return defaults;

  if (!isPojo(options)) {
    throw new TypeError(msg.argOptionsPojo);
  }

  defaults.dispose = elv.coalesce(options.dispose, defaults.dispose);

  if (typeof defaults.dispose !== 'boolean') {
    throw new TypeError(msg.argOptionsDisposeBool);
  }
...
// The disposer function to use when a Connector is configured to auto-dispose
// client instances returned by connect().
//
function disposer(client) {
  client.release();
}
...
class Connector {
  constructor(pgLib, options) {
    ...
    const opts = assertConnectorOptions(options);
  ...
  connect(repository, callback) {
    ...
    //
    const result = new Promise(promiseFn);
    return (this._dispose) ? result.disposer(disposer) : result;
  }
  ...
  load(providers, value, callback) {
    ...
    configurator
      .on('config', (fragment) => {
        this._emitter.emit('config', fragment);
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      configurator.load(providerFns, (err, conf) => {
        ...
        this._emitter.emit('done', conf);
        if (elv(cbfn)) cbfn(null, this);

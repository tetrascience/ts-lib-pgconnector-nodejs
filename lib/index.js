...
class Connector {
   ...
   * @private
   */
  static _assertCallback(callback) {
   ...
   * @private
   */
  static _isOptionalInt(value) {
    return !elv(value) || Number.isInteger(value);
  }
   ...
   * @private
   */
  static _assertRepo(repository) {
    if (!Connector._isNonEmptyString(repository)) {
  ...
  //
  // Handle configuration, and hydrate Connector
  //
  ...
  static _validateConf(conf) {
    ...
    if (!Connector._isPojo(conf.repositories)) {
   ...
   * @private
   */
  static _validateDatabaseConf(key, dbConf) {
    if (!elv(dbConf) || !Connector._isPojo(dbConf)) {
    ...
    if (!Connector._isNonEmptyString(dbConf.host)) {
  ...
  _openPool(dbConf) {
    const conf = dbConf;
    const pool = new this._pg.Pool(conf);
    const self = this;

    self.on('error', (err, client) => {
      self._emitter.emit('error', err, client, conf);
    });
  ...
  _fillDatabasesMap(databases) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const databaseErr = Connector._validateDatabaseConf(key, dbConf);
      if (elv(databaseErr)) return databaseErr;

      const pool = this._openPool(dbConf);
  ...
  _validateRepoConf(key, repoConf) {
    if (!Connector._isNonEmptyString(repoConf)) {
      return new ConfigurationError(msg.repoConfStr, key);
    }

    if (!this._databases.has(repoConf)) {
  ...
  _fillReposMap(repositories) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      repos.set(key, {
        name: key,
        pool: this._databases.get(repoConf).pool,
   ...
   */
  _fill(conf) {
    const validateErr = Connector._validateConf(conf);
  ...
  connect(repository, callback) {
    ...
    // Promise.reject(), because this is problem with the code itself.  The
    // app should cease running.
    //
    Connector._assertRepo(repository);
    Connector._assertCallback(callback);

    const repo = this._repositories.get(repository);
    const cbfn = callback;
    const self = this;
  ...
  load(providers, value, callback) {
    ...
    Connector._assertCallback(cbfn);
    ...
    return new Promise((resolve, reject) => {
      ...
      configurator.load(providerFns, (err, conf) => {
        ...
        if (elv(fillErr)) {
          self._emitter.emit('error', fillErr);
